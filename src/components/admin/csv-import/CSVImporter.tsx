...
import { generateSeoFields } from './seoUtils';
  ...
  const handleValidation = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < csvData.length; i++) {
        ...
        const metaTitleProvided = !!(productData.meta_title && String(productData.meta_title).trim());
        const metaDescriptionProvided = !!(productData.meta_description && String(productData.meta_description).trim());
        
        const seoFields = generateSeoFields(productData);
        if (!metaTitleProvided) productData.meta_title = seoFields.meta_title;
        if (!metaDescriptionProvided) productData.meta_description = seoFields.meta_description;
        ...
        products.push({
          ...
          errors,
          seoAutoGenerated: {
            meta_title: !metaTitleProvided,
            meta_description: !metaDescriptionProvided,
          },
        });
      }
